...
woh_hai<-list.files("C:\\kindle_patrika\\PDB\\pdb_MoserFear\\")  #all stationary files


for ( bari_bari in woh_hai){
vaishakhi<-substr(bari_bari,1,nchar(bari_bari)-16) #contains file name
pyar<-read.table(paste("C:\\kindle_patrika\\PDB\\pdb_MoserFear\\",bari_bari,sep=""),skip=1 )  #opening potential file
...
write.table(behind,paste("C:\\Users\\msc2\\Desktop\\Halwa\\BADMASHI\\forecasting\\AR\\forecast_AR_stationary\\",vaishakhi,".txt",sep=""),col.names=c("ori_pot","pred_pot","R_Square","ori_AA","pred_AA","ori_states","pred_states"),row.names=F)
...
if(!is.null(ohm)){write.table(ohm,"C:\\Users\\msc2\\Desktop\\Halwa\\BADMASHI\\forecasting\\AR\\AR_stat_p_zero_models.txt",col.names="P-value zero files",row.names=F)}

#writing info file
write.table(dhino_dhino,"C:\\Users\\msc2\\Desktop\\Halwa\\BADMASHI\\forecasting\\AR\\AR_forecast_stat_R_sq_info.txt",col.names=c("PDB_id","Gen_R_square","Wiki_R_square"),row.names=F)
